...
class ModuleMake extends Command
{
    ...
    private function updateModularConfig() {   ///дозапись части кода в файл php
        $group = explode('\\', $this->argument('name'))[0];
        $module = Str::studly(class_basename($this->argument('name')));
        $modular = $this->files->get(base_path('config/modular.php'));   ///получаем контент файла php

        $matches = [];
        preg_match("/'modules' => \[.*?'{$group}' => \[(.*?)\]/s", $modular, $matches);  ///поиск по регулярному выражению
        /// первый аргумент шаблон регулярного выражения второй-строка где осуществляется поиск третий -куда записывается результат
        print_r($matches);
        if(count($matches) == 2) {
            if(!preg_match("/'{$module}'/", $matches[1])) {
                $parts = preg_split("/('modules' => \[.*?'{$group}' => \[)/s", $modular, 2, PREG_SPLIT_DELIM_CAPTURE);///разделение
                /// определенной строки
                if(count($parts) == 3) {
                    $configStr = $parts[0].$parts[1]."\n            '$module',".$parts[2];
                    $this->files->put(base_path('config/modular.php'), $configStr);   ///запись в файл php
                }
            }
        }
